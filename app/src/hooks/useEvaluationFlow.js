...
export function useEvaluationFlow(initialLogId = null) {
  ...
  const handleModel = () => {
    ...
    if (log.log_type !== model.log_type) {
      ...
      setModelError(
        i18next.t("incompatibleModel", {
          log_type: log.log_type,
          log_type2: model.log_type,
        }),
      );
      setLoading(false);
      return;
  ...
  const handleDecisionBoundary = async () => {
    ...
    try {
      ...
      const blob = await evaluationsApi.getEvaluationImage(selectedEvaluation);
      ...
      const anomaliesData = await anomaliesApi.getAnomalies(selectedEvaluation);
