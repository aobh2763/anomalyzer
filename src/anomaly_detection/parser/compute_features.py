...
def compute_deltatime(timestamps):
    """Compute deltatime between consecutive, already-sorted timestamps. The first row has no predecessor (NaT by default), filled with the mean of all other deltatimes instead, so it doesn't break downstream numeric handling.
...
def compute_sequence_features(rows):
    """Compute previous_event_id and event_id_frequency for a list of already-sorted flat rows, mutating them in place. Both are None-only placeholders before this call.
